"""Tests for CORE source integration.

Note: CORE API has strict rate limits without an API key:
- 100 requests/day, 10/minute for unauthenticated requests
- Set CORE_API_KEY environment variable for higher limits
...
def test_search_papers_returns_list() -> None:
    ...
    results = search_papers(topic="open access publishing", max_results=2)
...
def test_search_papers_authors_extraction() -> None:
    """Test that authors are properly extracted."""
    results = search_papers(topic="machine learning applications", max_results=3)
...
def test_search_papers_abstract_extraction() -> None:
    ...
    results = search_papers(topic="renewable energy", max_results=3)
...
def test_search_papers_url_generation() -> None:
    """Test that URLs are properly generated (DOI > downloadUrl > CORE)."""
    results = search_papers(topic="open data", max_results=5)
...
def test_search_papers_publication_date_format() -> None:
    ...
    results = search_papers(topic="digital humanities", max_results=3)
...
def test_search_papers_open_access_content() -> None:
    """Test that CORE returns open access content."""
    # CORE specializes in open access, should return results
    results = search_papers(topic="biodiversity conservation", max_results=5)
